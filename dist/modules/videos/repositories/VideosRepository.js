    ...
    create(request, response) {
        ...
        mysql_1.pool.getConnection((error, connection) => {
            connection.query('INSERT INTO videos (video_id, user_id, title, description) VALUES (?,?,?,?)', [(0, uuid_1.v4)(), user_id, title, description], (error, result, fileds) => {
                ...
                response.status(200).json({ message: 'Video adicionado com sucesso' });
            });
        });
    }
    getVideos(request, response) {
        ...
        mysql_1.pool.getConnection((error, connection) => {
            connection.query('SELECT * FROM videos WHERE user_id = ?', [user_id], (error, results, fileds) => {
                connection.release();
                if (error) {
                    return response.status(400).json({ error: 'Pesquisa sem resultados!!' });
                }
                return response.status(200).json({ message: 'Encontrado com sucesso', videos: results });
            });
        });
    }
